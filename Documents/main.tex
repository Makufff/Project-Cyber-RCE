...
\begin{enumerate}
    ...
    \item เมื่อพบ Property Value ที่เป็น String ขึ้นต้นด้วย \texttt{"\$"} เรียก \texttt{resolveReference()} เพื่อ Resolve
    \item \texttt{resolveReference()} แยกวิเคราะห์ format เช่น \texttt{"\$1:a:b"} ดึง Chunk 1 แล้ว traverse \texttt{a.b}
    \item หลังจาก Resolve ทุก Reference แล้ว ตรวจสอบว่า Object มี \texttt{.then} property หรือไม่
    \item ถ้ามี \texttt{.then} ถือว่าเป็น Promise พยายาม Resolve Promise นั้น
\end{enumerate}
...
\texttt{"\$1:constructor:constructor"} Chunk1.constructor.constructor \textbf{Function} class นั่นเอง ตอนนี้ Deserializer สามารถสร้าง Function จาก string ได้แล้ว
...
\node[box, fill=orange!20, below=of s2] (s3) {ขั้นที่ 3: Resolve \texttt{\$1:\_\_proto\_\_:then} Prototype Pollution};
\node[box, fill=red!20,    below=of s3] (s4) {ขั้นที่ 4: Resolve \texttt{\$1:constructor:constructor} Function};
\node[box, fill=red!30,    below=of s4] (s5) {ขั้นที่ 5: Run \texttt{\_prefix} เป็น JavaScript};
\node[box, fill=green!20,  below=of s5] (s6) {ขั้นที่ 6: รับ \texttt{digest} ใน Response Base64 Decode};
...
แต่เมื่อนำมาประกอบกัน RCE การทดสอบ Security จึงต้องคิดถึง Composition ของ features ไม่ใช่แค่แต่ละ feature ทีละชิ้น
